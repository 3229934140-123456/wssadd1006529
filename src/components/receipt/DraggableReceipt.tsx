...
export default function DraggableReceipt({
  ...
  const handleDragStart = useCallback(
    (e: React.DragEvent) => {
      ...
      e.dataTransfer.setData(
        'application/json',
        JSON.stringify({ type: 'receipt', id: receipt.id })
      );
      onDragStart?.(e, receipt);
    },
    [receipt, onDragStart]
  );
  ...
  return (
    <div
      ...
      className={cn(
        'group relative bg-white rounded-xl overflow-hidden shadow-sm border transition-all duration-300',
        !isMatched && 'cursor-grab active:cursor-grabbing',
      ...
      >
        ...
        <div className="absolute inset-0 px-4 py-3 flex items-start justify-between">
          ...
          <div className="flex items-center gap-2">
            {!isMatched && (
              <div className="p-1 rounded bg-white/20 backdrop-blur-sm text-white">
        ...
        <div className="space-y-2 pt-3 border-t border-dashed border-gray-200">
          ...
          {receipt.note && (
            <div className="pt-1 mt-1 border-t border-gray-100">
              <p className="text-xs text-gray-500 line-clamp-2">{receipt.note}</p>
            </div>
          )}
        </div>
      </div>
